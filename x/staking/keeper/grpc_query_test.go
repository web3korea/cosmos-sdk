...
func createValidators(t *testing.T, ctx sdk.Context, app *simulateapp.SimApp, powers []int64) ([]sdk.AccAddress, []sdk.ValAddress, []types.Validator) {
	addrs := simulateapp.AddTestAddrsIncremental(app, ctx, 5, app.StakingKeeper.TokensFromConsensusPower(ctx, 300))
	valAddrs := simulateapp.ConvertAddrsToValAddrs(addrs)
	pks := simulateapp.CreateTestPubKeys(5)
