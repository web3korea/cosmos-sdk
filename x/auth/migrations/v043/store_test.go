...
func TestMigrateVestingAccounts(t *testing.T) {
	testCases := []struct {
		name        string
		prepareFunc func(app *simulateapp.SimApp, ctx sdk.Context, validator stakingtypes.Validator, delegatorAddr sdk.AccAddress)
		garbageFunc func(ctx sdk.Context, vesting exported.VestingAccount, app *simulateapp.SimApp) error
		tokenAmount int64
		expVested   int64
		expFree     int64
		blockTime   int64
	}{
		{
			...
			func(app *simulateapp.SimApp, ctx sdk.Context, validator stakingtypes.Validator, delegatorAddr sdk.AccAddress) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			addrs := simulateapp.AddTestAddrs(app, ctx, 1, sdk.NewInt(tc.tokenAmount))
...
func cleartTrackingFields(ctx sdk.Context, vesting exported.VestingAccount, app *simulateapp.SimApp) error {
...
func dirtyTrackingFields(ctx sdk.Context, vesting exported.VestingAccount, app *simulateapp.SimApp) error {
...
func createValidator(t *testing.T, ctx sdk.Context, app *simulateapp.SimApp, powers int64) (sdk.AccAddress, sdk.ValAddress) {
	valTokens := sdk.TokensFromConsensusPower(powers, sdk.DefaultPowerReduction)
	addrs := simulateapp.AddTestAddrsIncremental(app, ctx, 1, valTokens)
	valAddrs := simulateapp.ConvertAddrsToValAddrs(addrs)
	pks := simulateapp.CreateTestPubKeys(1)
