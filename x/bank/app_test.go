...
func TestSendNotEnoughBalance(t *testing.T) {
	...
	app := simulateapp.SetupWithGenesisAccounts(t, genAccs)
	...
	_, _, err := simulateapp.SignCheckDeliver(t, txGen, app.BaseApp, header, []sdk.Msg{sendMsg}, "", []uint64{origAccNum}, []uint64{origSeq}, false, false, priv1)
	require.Error(t, err)

	simulateapp.CheckBalance(t, app, addr1, sdk.Coins{sdk.NewInt64Coin("foocoin", 67)})
...
func TestMsgMultiSendWithAccounts(t *testing.T) {
	...
	for _, tc := range testCases {
		header := tmproto.Header{Height: app.LastBlockHeight() + 1}
		txGen := simulateapp.MakeTestEncodingConfig().TxConfig
		_, _, err := simulateapp.SignCheckDeliver(t, txGen, app.BaseApp, header, tc.msgs, "", tc.accNums, tc.accSeqs, tc.expSimPass, tc.expPass, tc.privKeys...)
		if tc.expPass {
		...
		for _, eb := range tc.expectedBalances {
			simulateapp.CheckBalance(t, app, eb.addr, eb.coins)
		}
	}
}
